...
/*
 ...
 6) Add the sensor value to the sVals array
...
// Define the sensor codes
#define ltCode 1
#define mstCode 2
#define tmpCode 3
// 1) Add more sensor codes here.
//    Define the sensor code in the web.config file of the web application
// const int [name] = 4;
...
/*Button idBtn = Button(2, &idHandler);
Button idUpBtn = Button(3, &idUpHandler);
Button idDownBtn = Button(4, &idDownHandler);
...
void loop()
{
  ...
  double ltVal = map(analogRead(ltPin), 0, 1023, 0, 100);

  // Get the soil moisture sensor value
  double mstVal = map(analogRead(mstPin), 1023, 0, 0, 100);

  // Get the temperature sensor value
  int tmpVal = getCelsius(analogRead(tmpPin));
  ...
  data[1] = id[1];
  data[2] = id[2];
  data[3] = sNums[sensorPosition];
  data[4] = sCodes[sensorPosition];
  data[5] = sVals[sensorPosition];
...
void idUpHandler(Button* button)
{
  if (button->pressed){
    id[editIdPosition]++;
...
void idDownHandler(Button* button)
{
  if (button->pressed){
    id[editIdPosition]--;
